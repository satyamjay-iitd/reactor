...
impl Iterator for BallotIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        // Start a new ballot every 0 - 1s
        // let s: u8 = random();
        // std::thread::sleep(Duration::from_millis(200 * s as u64));
...
impl Leader {
    ...
    fn propose(&mut self) -> Vec<PaxosMsg> {
        info!("Before self.max_val: {:?}, self.proposed_val: {:?}", self.max_val, self.proposed_val);
        if self.proposed_val.is_none() {
            if 2 * self.max_val.len() > self.acceptors.len() {
                // We have heard back from the majority.
                let mut val_counts: HashMap<Option<Val>, u8> = HashMap::new();
                for v in self.max_val.values() {
                    val_counts.entry(*v).and_modify(|c| {*c = *c + 1}).or_insert(1);
                }
        ...
        match input {
            PaxosMsg::A1(l, b) => {
                ...
                if self.consensus_val.is_none() {
                    ...
                    info!("{} ignoring A1 message since consensus is reached on value {}", self.addr, self.consensus_val.unwrap());
                    vec![]
                }
            }
            PaxosMsg::B1(acceptor, leader, b, last_ballot, last_val) => {
                info!("{} received B1 from {}", self.addr, acceptor);
                std::thread::sleep(Duration::from_millis(1000));
                assert_eq!(leader, self.addr);
                if self.ballot == b {
                    if let Some(ballot) = last_ballot {
                        if ballot > self.ballot {
                            info!("Got a higher ballot {ballot} from {acceptor} than my current ballot {}", self.ballot);
                            return vec![];
                        }
                    }
            ...
            PaxosMsg::B2(_acceptor, leader, b, val) => {
                info!("{} received B2 from {}", self.addr, _acceptor);
                std::thread::sleep(Duration::from_millis(1000));
                assert_eq!(leader, self.addr);
                if self.ballot == b {
                    assert_eq!(val, self.proposed_val.unwrap(),
                               "Acceptor voted for {val} but I had proposed {}", self.proposed_val.unwrap());
                    self.vote_count += 1;
                }
                if 2 * self.vote_count > self.acceptors.len() as u8 {
                    ...
                    info!("{} Have not reached consensus yet. Current votes: {}", self.addr, self.vote_count);
                }
                vec![]
            }
...
pub async fn actor(ctx: RuntimeCtx, acceptors: Vec<AcceptorAddr>) {
    BehaviourBuilder::new(Leader::new(ctx.addr, acceptors.clone()), BincodeCodec::default())
